...
enum BioFeedbackMagicSchool: Int {
    case mysticArt = 0
    case thunder = 1
    case harryPotter = 2
    case jLaser = 4
...
enum BioFeedbackWatchState: Int {
    case normal = 0
    case ground = 1
}

enum BioFeedbackHandedness: Int {
    case right = 0
    case left = 1
}

class BioFeedbackWatchAppManager: NSObject, ObservableObject {
    
    @Published var view: BioFeedbackView = .readyView
    
    @Published var magicSchool: BioFeedbackMagicSchool = .mysticArt
    
    @Published var handedness: BioFeedbackHandedness = .right {
        didSet {
    ...
    // Round result stats
    @Published var roundResult: BioFeedbackRoundResult = .victory
    ...
    var currentState: BioFeedbackWatchState = .normal
    ...
    override init() {
        ...
        if UserDefaults.standard.object(forKey: handednessKey) != nil  {
            self.handedness = BioFeedbackHandedness(rawValue: UserDefaults.standard.integer(forKey: handednessKey)) ?? .right
        }
    }
    ...
    func sendStartRoundMessage() {
        let message = ["BioFeedback" : true,
                       "Start": true]
        CellSpaceWatchAppManager.shared.wcSession.sendMessage(message, replyHandler: nil)
        startWorkout()
    }
    ...
    func sendWatchStateChangedMessage(watchState: BioFeedbackWatchState) {
        let message = ["BioFeedback" : true,
...
extension BioFeedbackWatchAppManager {
    func didReceiveApplicationContext(applicationContext: [String : Any]) {
        if applicationContext["Start"] is Bool {
            if let magicSchool = applicationContext["MagicSchool"] as? Int {
                DispatchQueue.main.async {
                    self.magicSchool = BioFeedbackMagicSchool(rawValue: magicSchool)!
                }
            }
        ...
        if applicationContext["End"] is Bool {
            if (self.view == .fightingView) {
                ...
                if let roundResultIndex = applicationContext["Result"] as? Int {
                    if let roundResult = BioFeedbackRoundResult(rawValue: roundResultIndex) {
                        DispatchQueue.main.async {
...
extension BioFeedbackWatchAppManager: HKWorkoutSessionDelegate {
...
extension BioFeedbackWatchAppManager: HKLiveWorkoutBuilderDelegate {
